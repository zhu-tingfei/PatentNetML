...
def main():
    ...
    wdirs = f'../results/xgboost_all'#time.strftime("%Y-%m-%d-T%H-%M-%S", time.localtime())
    ...
    with open(results_file,'w+') as f:
        f.write(','.join(['rounds','max_depth','n_estimators','learning_rate','subsample','min_child_weight',
    ...
    for r in range(max_r,rounds):
        ...
        with open(log_file,'w+') as f:
            f.write(','.join(['max_depth', 'n_estimators','learning_rate', 'subsample','min_child_weight','valid_roc','test_roc'])+'\n')
        # model training
        def f(max_depth,n_estimators,learning_rate,subsample,min_child_weight,direction = False):
            clf = xgboost.XGBClassifier(random_state=123,nthread=12,scale_pos_weight=negative_sample_num/1,
                                        max_depth=int(round(max_depth)),n_estimators=int(round(n_estimators)),
                                        learning_rate=10**-learning_rate,subsample=subsample,
                                        min_child_weight=int(round(min_child_weight)))

            clf.fit(X_train, y_train,eval_metric="auc",eval_set=[(X_valid, y_valid)],early_stopping_rounds=10,verbose=True)
            ...
            with open(log_file,'a') as f:
                f.write(','.join([str(int(round(max_depth))), str(int(round(n_estimators))), str(learning_rate),str(subsample),str(int(round(min_child_weight)))]))
    ...
    ### Bayesian optimization
        ...
        param = {
                 ...
                 'learning_rate': ('cont', [0, 5]),
                 'subsample': ('cont', [0.5, 0.9]),
                 'min_child_weight':('int', [1, 10])
                 }
        np.random.seed(168)
        gpgo = GPGO(gp, acq, f,param,n_jobs=12)
        gpgo.run(max_iter=100,init_evals=3)
        results = gpgo.getResult()
        print(results)
        best_param = results[0]
        best_result = results[1]
        # best
        best_max_depth = best_param.get('max_depth')
        best_n_estimators = best_param.get('n_estimators')
        best_learning_rate = best_param.get('learning_rate')
        best_subsample = best_param.get('subsample')
        best_min_child_weight = best_param.get('min_child_weight')
        clf = xgboost.XGBClassifier(random_state=123,nthread=12,
                                    scale_pos_weight=negative_sample_num/1,
                                    max_depth=best_max_depth,
                                    n_estimators=best_n_estimators,
                                    learning_rate=10**-best_learning_rate,
                                    subsample=best_subsample,
                                    min_child_weight=best_min_child_weight)
        clf.fit(X_train, y_train,eval_metric="auc",eval_set=[(X_valid, y_valid)],early_stopping_rounds=10,verbose=True)
        with open(f'{wdirs}/xgboost_{r}.pkl', 'wb+') as f:
            pickle.dump(clf, f)
        ...
        with open(results_file,'a') as f:
            f.write(','.join([str(r),str(best_max_depth),str(best_n_estimators),str(best_learning_rate),str(best_subsample),str(best_min_child_weight),
